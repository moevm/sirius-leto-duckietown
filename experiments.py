...
from plot import color_segment
from plot import drawLines
...
def update(dt):
    ...
    #    edges.save(f"screen_{env.unwrapped.step_count / 10}.png")
        ...
        rets = detector.detectLines('yellow')
        lines = rets[0]
        normals = rets[1]
        bw = rets[2]
        centers = rets[3]
        ...
        # im = res.reshape((im.shape))
        # cv2.imwrite(f"screenshots/screen_{env.unwrapped.step_count / 10 + 0.1}.png", im)
        # for i in range(im.shape[0]):
        #     for j in range(im.shape[1]):
        #         if not((im[i][j][0] < 170) and (im[i][j][1] > 140) and (im[i][j][2] > 140)):
        #             im[i][j] = [0,0,0]
        #cv2.imwrite(f"screenshots/screen_{env.unwrapped.step_count / 10}.png", im)
    #    img_gray = im.convert('L')cv
